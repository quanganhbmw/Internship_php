...
$row = mysqli_fetch_array($data_cat);

if(isset($_POST['sbm'])) {
    $cat_name = $_POST['cat_name'];
    $sql_update_cat = "SELECT * FROM category WHERE cat_name ='$cat_name'";
    $data_update_cat = mysqli_query($conn, $sql_update_cat);
    $num_row = mysqli_num_rows($data_update_cat);
    if($num_row > 0) {
